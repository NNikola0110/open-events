...
export async function getEventsSorted(req: Request, res: Response) {
  try {
    const search = (req.query.search as string) || "";
    const page = parseInt(req.query.page as string) || 1;
    const limit = parseInt(req.query.limit as string) || 10;
    const sort = (req.query.sort as string) || "recent"; // "recent" | "views"
    const skip = (page - 1) * limit;
    ...
    const order =
      sort === "views"
        ? ({ numberOfViews: "DESC", createDate: "DESC" } as const)
        : ({ createDate: "DESC" } as const);
